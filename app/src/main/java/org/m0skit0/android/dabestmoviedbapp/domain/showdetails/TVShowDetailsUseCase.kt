...
typealias TVShowDetailsUseCase = suspend (state: ApplicationState) -> Either<Throwable, TVShowDetailsState>

suspend fun tvShowDetailsUseCase(
    state: ApplicationState,
    repository: TVShowDetailsRepository = koin().get(NAMED_TV_SHOW_DETAILS_REPOSITORY),
): Either<Throwable, TVShowDetailsState> = withUseCaseContext { repository(state) }
