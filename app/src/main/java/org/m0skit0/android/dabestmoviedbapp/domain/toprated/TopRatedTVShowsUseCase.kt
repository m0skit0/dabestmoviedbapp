...
typealias TopTVShowsUseCase = suspend (state: ApplicationState) -> Either<Throwable, TopRatedTVShowsState>

suspend fun topTVShowsUseCase(
    state: ApplicationState,
    repository: TopRatedTVShowsRepository = koin().get(NAMED_TOP_TV_SHOWS_REPOSITORY)
): Either<Throwable, TopRatedTVShowsState> = withUseCaseContext { repository(state) }
